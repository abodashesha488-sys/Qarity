...
/// شريط حالة مصغّر أعلى «نظرة عامة» — يعرض ما هو معلّق الآن مع اختصار للتبويب.
class _LiveAlertSummary extends StatelessWidget {
  const _LiveAlertSummary({required this.onOpenAlerts});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          _miniStatus(svc.watchLiveAlert(), 'تنبيه', const Color(0xFFC62828),
              Icons.campaign_rounded),
          const SizedBox(width: 10),
          _miniStatus(svc.watchLiveBreaking(), 'خبر عاجل',
              const Color(0xFFF9A825), Icons.bolt_rounded),
          const Spacer(),
          TextButton.icon(
  ...
  Widget _miniStatus(
      Stream<VillageAlert?> stream, String label, Color color, IconData icon) {
    return StreamBuilder<VillageAlert?>(
      stream: stream,
      builder: (context, snap) {
        final live = snap.data?.liveAt(DateTime.now()) ?? false;
        return Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(live ? icon : Icons.notifications_off_rounded,
                size: 15,
                color: live
                    ? color
                    : Theme.of(context)
            ...
            Text(
              '$label: ${live ? 'مُفعّل' : 'متوقف'}',
              style: TextStyle(
                  fontSize: 11.5,
                  fontWeight: FontWeight.w800,
                  color: live ? color : Colors.black45),
            ),
          ],
        );
      },
